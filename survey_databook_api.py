...
def is_bipolar_sub_label(first_cell, next_row):
    ...
    first = str(first_cell).strip()
    if " or " not in first.lower():
        return False
    ...
    labels = []
    for c in range(1, min(10, len(next_row))):
        val = next_row.iloc[c]
        if pd.isna(val):
            continue
        ...
        if val_str.lower() == "total":
            break
        if val_str.lower() in {"answered", "skipped", "responses"}:
            return False
...
def get_auto_type(q_text, rank_labels, options, is_bipolar=False):
    ...
    MULTIPLE_KEYWORDS = [
        ...
        "select all that apply", "select all which apply",
        "all that apply", "all which apply",
        "tick all", "check all",
        "select all that are applicable", "select all applicable"
    ]
    ...
    if re.search(r'\b(select|choose|pick|mention|state)\s+up\s*to\s+\d+', q_lower):
        return "Multiple"

    m = re.search(r'\b(select|choose|pick|mention|state)\s+(?:top\s+)?(\d+)\b', q_lower)
...
def clean_and_convert(input_path, output_path):
    ...
    if missing_type_questions:
        ...
        for q in missing_type_questions:
            ...
            while True:
                user_input = input("Enter type (Single/Multiple/Bipolar): ").strip().capitalize()
                if user_input in ["Single", "Multiple", "Bipolar"]:
                    q["manual_type"] = user_input
                    break
                else:
                    print("❌ Invalid input. Please enter Single or Multipl or Bipolar.")
...
class SurveyDatabookV2:
    ...
    def identify_structure(self):
        known_questions = set(self.question_config.keys())
        ...
        for idx, col in enumerate(self.df.columns):
            ...
            if 'unnamed' in col_lower:
                if found_first_question and current_question is not None:
                    self.question_map[current_question]['unnamed_cols'].append(idx)
                continue

            if col_str in known_questions:
        ...
        print(f"   First question starts at col index: "
              f"{self.question_map[1]['main_col_idx'] + 1 if self.question_map else '?'}")
    ...
    def create_databook(self, output_path, survey_name="Survey Databook"):
        ...
        for question in analysis['questions']:
            ...
            ws[f'A{row}'] = f"Type: {question['question_type'].title()}"
            ...
            if is_bipolar:
                ...
                headers = ['Response Option', 'Pole 1', 'Pole 2', 'N', '% Pole 1', '% Pole 2']
            ...
            # ── SINGLE / MULTIPLE TABLE ────────────────────────────────────
                ...
                for idx, item in enumerate(question['data']):
                    ...
                    if is_multiple and idx < len(all_col_letters):
                        ...
                        if is_other:
                            count_formula = (
                f"=COUNTA('Raw Data'!${option_col}$3"
                f":${option_col}$8000)"
                            )
                        else:
                            count_formula = (
        ...
        if is_bipolar:
            ...
            for cb in cut_blocks:
                ...
                for data_row in range(first_opt, last_opt + 1):
                    ...
                    pct1.border = border_thin
                    pct1.alignment = center;
                    pct1.font = Font(name="Arial")
                    ...
                    pct2.border = border_thin
                    pct2.alignment = center;
                    pct2.font = Font(name="Arial")
            ...
            for cb in cut_blocks:
                ...
                for row_offset in range(last_opt - header_row + 1):
                    ...
                    for col_offset in range(total_matrix_cols):
                        ...
                        pct_zone_start_offset = num_rating_cols + 1
                        is_pct_col = col_offset >= pct_zone_start_offset
                        is_header_row = (src_row == header_row)
                        if is_pct_col and not is_header_row:
                            rating_idx = col_offset - pct_zone_start_offset
                            cut_count_col = get_column_letter(start_col + rating_idx)
                            cut_n_col = get_column_letter(start_col + num_rating_cols)
                            dest_cell.value = f"=IFERROR({cut_count_col}{src_row}/{cut_n_col}{src_row}*100,0)"
                            dest_cell.number_format = '0.0"%"'
                            if src_cell.has_style:
                                dest_cell._style = copy(src_cell._style)
                            dest_cell.number_format = '0.0"%"'
                        else:
...
def _parse_questions_from_file(file_path: str) -> list:
    ...
    current_rank_labels = []
    current_is_bipolar = False
    current_bipolar_dimensions = []
    current_bipolar_poles = []
    in_question = False

    def _flush():
        if current_q_text and (current_options or current_rank_labels or current_bipolar_dimensions):
            ...
            questions.append({
                ...
                "rank_labels": current_rank_labels.copy(),
                "is_bipolar": current_is_bipolar,
                "bipolar_dimensions": current_bipolar_dimensions.copy(),
                "bipolar_poles": current_bipolar_poles.copy(),
            })
    ...
    while idx < len(df):
        ...
        if first_cell == "":
            if not current_bipolar_dimensions:
                _flush()
                current_q_text = None
                current_options = []
                current_rank_labels = []
        ...
        if is_question_row(first_cell):
            ...
            # Bipolar check — dimensions collected row by row below
        ...
        # Inside a question — collect options or bipolar dimensions
        elif in_question and not current_rank_labels:
            if not is_metadata_text(first_cell) and first_cell.lower() != "answer choices":
                if idx + 1 < len(df) and is_bipolar_sub_label(first_cell, df.iloc[idx + 1]):
                    current_bipolar_dimensions.append(first_cell)
                    if not current_bipolar_poles:
                        poles_row = df.iloc[idx + 1]
                        poles = []
                        for c in range(1, min(10, len(poles_row))):
                            val = poles_row.iloc[c]
                            if pd.notna(val):
                                val_str = str(val).strip()
                                if val_str.lower() == "total":
...
def _write_options_file(questions: list, confirmed_types: dict, output_path: str):
    ...
    for q in questions:
        ...
        # If bipolar, use bipolar_dimensions; otherwise use normal options
        if q_type.lower() == "bipolar" and q.get("bipolar_dimensions"):
            opts_to_write = q["bipolar_dimensions"]
        else:
            opts_to_write = q["options"]

        for opt in opts_to_write:
            ws.cell(row, 1, q["q_text"]).font = Font(name="Arial")
            ws.cell(row, 2, clean_value(opt)).font = Font(name="Arial")
            ws.cell(row, 3, q_type).font = Font(name="Arial")
            for r_idx, rank in enumerate(q["rank_labels"]):
...
def _build_cuts_template(cuts: List[CutConfigItem], session_dir: str) -> str:
    """
    Converts the API cuts list into a temp Template_Cuts.xlsx.
    Now takes session_dir as parameter to avoid collisions between concurrent users.
    ...
    # Save in session-specific folder to avoid conflicts
    cuts_path = os.path.join(session_dir, "cuts_template.xlsx")
    wb.save(cuts_path)
    return cuts_path
...
async def generate_databook(
    ...
    **Validates ownership**: User must own the session they're trying to generate.
    ...
    try:
        ...
        if request.cuts:
            session_dir = os.path.dirname(options_path)
            cuts_temp_path = _build_cuts_template(request.cuts, session_dir)
